...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!response.ok) return res.status(response.status).json({ error: data?.error?.message || "Polling failed" });
    if (!data.done) return res.status(200).json({ status: "processing" });
    if (data.error) return res.status(500).json({ error: data.error.message });

    // Return the secure edge proxy route
    ...
    if (error.name === "AbortError") return res.status(504).json({ error: "Polling timed out" });
